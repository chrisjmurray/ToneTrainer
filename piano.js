    ...
    for (var o=startOctave; o<=endOctave; o++) {
        for (var k=keyReverseMap[startKey]; k<(o === endOctave ? keyReverseMap[endKey]+1 : keysLength); k++) {
            ...
            if (blackKeyMap[n] && !firstOccurrence) {
                ...
                let isWhiteRoot =""
                let isBlackRoot=""
                if (n == options.rootNote) {
                    isWhiteRoot = "True"
                }
                if (displayBlackKey == options.rootNote) {
                    isBlackRoot = "True"
                }
                keyElementArray.push('<li class="oct' + o + '"><div data-ipn="' + n+o + '" data-n="' + n + '" data-keyname="' +  displayWhiteKey + '" isRoot="'+isWhiteRoot+'" class="anchor key white ' + n+o + '"></div><span data-ipn="' + blackIpnName+o + '" data-keyname="' + displayBlackKey + '" isRoot="'+isBlackRoot+'" class="key black ' + blackNames.join(' ') + '"></span></li>');
            } else {
                let isWhiteRoot =""
                if (n == options.rootNote) {
                    isWhiteRoot = "True"
                }
                keyElementArray.push('<li class="oct' + o + '"><div data-ipn="' + n+o + '" data-n="' + n +'" data-keyname="' + displayWhiteKey + '" isRoot="'+isWhiteRoot+'" class="anchor key ' + n+o + '"></div></li>');
            }
            if (firstOccurrence) {
